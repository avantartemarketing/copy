...
hook on the announcement only, the beneficiary named once, and an action line that ends with the
...
Google insists on an exact match for that address, scheme, host and path; the app builds it from
Render's own address for the service, and `PUBLIC_URL` overrides that once the app has a custom
domain.
