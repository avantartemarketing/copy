...
for k, v, note in facts:
    ...
    R[k] = row; row += 1
row += 1; ws[f"A{row}"] = "Fragments, written once per release. No we or our: the sheet adds our where a line needs it."; ws[f"A{row}"].font = BOLD; row += 1
frags = [
...
rules = [
 ...
 ("Voice", "Fragments never say we or our, and the making line says 'printmakers at Make-Ready' so the sheet can add 'our'. The hook may name the artist. The Insiders email and the first-time survey are signed by the advisor named on Inputs. Features never say 'our'."),
